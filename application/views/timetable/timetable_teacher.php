...
<script type="text/javascript">
    $(document).ready(function(){
        ...
        $(".download").on("click",function(){
            location.href="index.php?d=timetable&c=timetable&m=out_teacher";
        });
    })
</script>
...
<div class="soft_function">
    ...
    <div class="soft_con">
        <div id="xzgl" class="soft_con2">
            ...
            <div class="function1" id="timetable_teacher" style="border-color: #1996E6;">
                <img src="static/timetable/images/pic_fun_project_detailed_003.gif"/>
                <p class="p1" style="background-color: #1996E6;font-size: medium;font-weight: bolder;">教师课表</p>
            </div>
...
<div class="row">
    <form method="post" action="<?=$this->baseurl?>&m=change_teacher">
        <ul class="list-inline form-group-sm">
            ...
            <li>
                <select class="form-control"  name="course" onchange="selectGrade()" style="width:120px;">
                    <?php foreach($course_all as $course_all_value): ?>
                        <option value="<?= $course_all_value;?>" <?php if($course==$course_all_value){echo "selected=selected";}?>><?= $course_all_value;?></option>
                    <?php endforeach;?>
                </select>
            </li>
            <li>
...
<div class="divDown">
    ...
    <div style="float: left; "><a href="javascript:void(0)" class="download" onclick="down()">导出教师课表</a></div>
</div>
<hr size="1" style="clear: both;width: 93%;margin-left: 20px;" >
<?php foreach($list as $class): ?>
    <div style="width: 93%;margin-left: 20px;">
        <h4 style="color: #00CC00;"><?=$class['teacher_truename']."：".$class['teach_course']?></h4>
                    ...
                    <?php foreach($weeks as $week_key=>$week):?>
                        <td>
                            <div>
                                ...
                                <span class="teacherName" courseid="1395101133226002" style="display: inline;"><?php if(!empty($class['table'][$section_key][$week_key]['grade'])){echo "(".$class['table'][$section_key][$week_key]['grade'].$class['table'][$section_key][$week_key]['classname'].")";} ?></span>
                            </div>
                        </td>
                    <?php endforeach;?>
                </tr>
