<?php
    ...
    foreach($list as $k=>$class):
        ...
        $objPHPExcel->getActiveSheet()->SetCellValue('A'.($k*$pp+1), '课程表');
        //$objPHPExcel->getActiveSheet()->mergeCells('A'.($k*10+2).':F'.($k*10+2));    //合并单元格：
        $objPHPExcel->getActiveSheet()->SetCellValue('B'.($k*$pp+2), $class['grade']);
        $objPHPExcel->getActiveSheet()->SetCellValue('C'.($k*$pp+2), $class['classname']);
        ...
        foreach($sections as $section_key=>$sction):
            ...
            foreach($weeks as $week_key=>$week):
                $objPHPExcel->getActiveSheet()->SetCellValue($letter[$week_key].($k*$pp+3+$section_key), $class['table'][$section_key][$week_key]['title']);
            endforeach;
        endforeach;
    endforeach;

    // Rename sheet
    $objPHPExcel->getActiveSheet()->setTitle('班级课程表');
    ...
    header("Content-Disposition:attachment;filename=班级课表.xls");
