...
<div class="mainbox nomargin">
    ...
    <form action="<?=$this->baseurl?>&m=detail" method="post">
        ...
        教师姓名：<input type="text" class="txt" id="teachername"
                    value="<?php echo $teacher['truename']?>" />
        <input type="hidden" class="txt" name="id" id="teacherid"
               value="<?php echo $teacher['id']?>" />&nbsp;&nbsp;
        <input type="submit" name="button" id="button" value="显示" class="btn"/>
        <input type="button"  value="饼图显示" class="btn" onclick="location.href='<?=$this->baseurl?>&m=chart&id=<?php echo $teacher['id']?>&pubdate=<?php echo $pubdate?>'"/>
        <hr />
    </form>
    <div class="cal">
