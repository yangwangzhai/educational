...
<div style=" margin:20px; font-size:13px;">
    ...
    <div class="row">
        <form method="post" action="<?=$this->baseurl?>&m=statistic_multiple_sub">
...
<div style="width: 82%;margin-left: 20px;">
    <div style="float: left;">
        <div id="main<?=$key?>_a" style="width: 500px;height:400px;border-right: dashed;float: left;"></div>
    </div>
    <div>
        <h4 style="color: #00CC00;margin-left: 150px;"><?=$score_first."至".$score_second;?>分数段历次考试人数折线图</h4>
        <div id="main<?=$key?>_b" style="width: 100%;height:400px;"></div>
    </div>
</div>
<script type="text/javascript">
    // 基于准备好的dom，初始化echarts实例
    var myChart = echarts.init(document.getElementById('main<?=$key?>_a'));
    // 指定图表的配置项和数据
    var option = {
        title : {
            text: "<?=$value['classname'].'：'.$score_first.'分至'.$score_second.'分所占比'?>",
            subtext: '<?=$subhead?>',
            x:'center'
        },
        tooltip : {
        ...
        series : [
            {
                ...
                data:[
                    {value:<?=$value['num_period']?>, name:'<?=$score_first.'至'.$score_second.'分人数'?>'},
                    {value:<?=$value['num_total']-$value['num_period']?>, name:'其他分数段人数'}
                ],
                itemStyle: {
                    emphasis: {
                        shadowBlur: 10,
...
<script type="text/javascript">
    // 基于准备好的dom，初始化echarts实例
    var myChart = echarts.init(document.getElementById('main<?=$key?>_b'));
    // 指定图表的配置项和数据
    var option = {
        tooltip : {
            trigger: 'axis'
