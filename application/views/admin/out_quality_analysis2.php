...
foreach($each_class_excellent as $k=>$class):
    ...
    $objPHPExcel->getActiveSheet()->SetCellValue('B'.($k*$pp+2), '0.9优秀率');
    $objPHPExcel->getActiveSheet()->SetCellValue('C'.($k*$pp+2), '0.8优秀率');
    $objPHPExcel->getActiveSheet()->SetCellValue('D'.($k*$pp+2),'及格优秀率');
    $objPHPExcel->getActiveSheet()->SetCellValue('E'.($k*$pp+2),'平均分');
    $objPHPExcel->getActiveSheet()->SetCellValue('F'.($k*$pp+2),'任课老师');

    foreach($class as $class_key=>$class_value):
        $objPHPExcel->getActiveSheet()->SetCellValue('A'.($k*$pp+3+$class_key),$class_value['classname']);
        $objPHPExcel->getActiveSheet()->SetCellValue('B'.($k*$pp+3+$class_key),$class_value['excellent_rate1']."%（".$class_value['excellent_rate1_num']."）");
        $objPHPExcel->getActiveSheet()->SetCellValue('C'.($k*$pp+3+$class_key),$class_value['excellent_rate2']."%（".$class_value['excellent_rate2_num']."）");
        $objPHPExcel->getActiveSheet()->SetCellValue('D'.($k*$pp+3+$class_key),$class_value['pass_rate']."%（".$class_value['pass_num']."）");
        $objPHPExcel->getActiveSheet()->SetCellValue('E'.($k*$pp+3+$class_key),$class_value['avg_score']);
        $objPHPExcel->getActiveSheet()->SetCellValue('F'.($k*$pp+3+$class_key),'');
    endforeach;
    $objPHPExcel->getActiveSheet()->SetCellValue('A'.($k*$pp+$pp),'年级');
    $objPHPExcel->getActiveSheet()->SetCellValue('B'.($k*$pp+$pp),$grade_excellent[$k]['excellent_rate1_grade']."%（".$grade_excellent[$k]['excellent_rate1_num_grade']."）");
    $objPHPExcel->getActiveSheet()->SetCellValue('C'.($k*$pp+$pp),$grade_excellent[$k]['excellent_rate2_grade']."%（".$grade_excellent[$k]['excellent_rate2_num_grade']."）");
    $objPHPExcel->getActiveSheet()->SetCellValue('D'.($k*$pp+$pp),$grade_excellent[$k]['pass_rate_grade']."%（".$grade_excellent[$k]['pass_num_grade']."）");
    $objPHPExcel->getActiveSheet()->SetCellValue('E'.($k*$pp+$pp),$grade_excellent[$k]['avg_score_grade']);
    $objPHPExcel->getActiveSheet()->SetCellValue('F'.($k*$pp+$pp),'');
endforeach;
