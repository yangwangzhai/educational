...
function initialize()
{
	...
	$("#deletebutton").click(function(){	
		if(confirm("确定该线段吗？")){
			var id = $("#id").val();
			if( id > 0 ) {
				$.get("index.php?d=admin&c=trafficmap&m=delete&id="+id,function(data){					
...
function submitform()
{
	...
	$.post("index.php?d=admin&c=trafficmap&m=save", { id:id, title:title, status:status, path:path, z_index:z_index, updatetime:updatetime }, function(dataid){		
		if (dataid) {			
...
function showline()
{	
	$.get("index.php?d=admin&c=trafficmap&m=list_all", function(data) {	
		data = eval('('+ data +')');
...
{		
	line.addEventListener('click', function(event) {	
		...
		g_activline = line;		
		$.get("index.php?d=admin&c=trafficmap&m=get_one&id="+id, function(data){
			data = eval('('+ data +')');
