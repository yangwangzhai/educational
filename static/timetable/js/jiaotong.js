...
function initialize()
{
	...
	$("#deletejiaotong").click(function(){	
		if(confirm("确定该线段吗？")){			
			var id = $("#jtid").val();			
			if( id > 0 ) {
				$.get("index.php?d=admin&c=jiaotong&m=delete&id="+id,function(data){					
...
function post_jiatong()
{
	...
	$.post("index.php?d=admin&c=jiaotong&m=save", { id:id, types:types, info:info,picture:picture, point:point, updatetime:updatetime }, function(dataid){		
		if (dataid) {		
			resetform();
			alert("成功！");		
		} 
...
function showpoint()
{	
	$.get("index.php?d=admin&c=jiaotong&m=list_lite", function(data) {	
		data = eval('('+ data +')');
...
{	
	marker.addEventListener('click', function(event) {
		g_marker = marker;		
		$.get("index.php?d=admin&c=jiaotong&m=get_one&id="+id, function(data){			
			data = eval('('+ data +')');
